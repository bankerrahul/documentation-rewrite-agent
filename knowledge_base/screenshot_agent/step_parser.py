"""
Step parser for Thrive Apprentice documentation markdown files.
...
def parse_action_and_target(step_text: str) -> Dict:
    ...
    # When the action is "toggle" and the target is just "On" or "Off",
    # the target is too ambiguous. Try to extract the actual feature name
    # from the raw text or context, and fall back to keeping "On"/"Off" as context.
    if action == "toggle" and target.lower() in ("on", "off"):
        ...
        if other_bolds:
            # Use the other bold item as the target (e.g., "Enable Comments for Courses")
            target = other_bolds[0]
            context = f"toggle {toggle_state}, {context}" if context else f"toggle {toggle_state}"
        else:
            # Try to extract the feature name from the raw text
            # E.g., "Switch it On to enable automatic login" → "auto-login"
            # But DON'T match generic phrases like "Toggle it On or Off based on..."
            # Look for "to enable <feature>" or "to disable <feature>" patterns
            ...
            if feature_match:
                ...
                if feature and feature.lower() not in ("it", "this", "the"):
                    ...
                    context = f"toggle {toggle_state}, {context}" if context else f"toggle {toggle_state}"
            # If still On/Off, keep as-is — capture.py will handle with section context
...
if __name__ == "__main__":
    ...
    docs_dir = sys.argv[1] if len(sys.argv) > 1 else "thrive_apprentice/new_docs"
    output_dir = sys.argv[2] if len(sys.argv) > 2 else "thrive_apprentice/screenshot_specs"
