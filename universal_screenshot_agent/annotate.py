...
Layout constants are read from the product config rather than hardcoded.
"""
...
ARROW_COLOR = (237, 28, 36)   # Bright saturated red
...
ARROWHEAD_LENGTH_CSS = 35      # Length of arrowhead triangle sides
...
def auto_arrow_position(
    ...
    Smart rules:
    1. Elements in the sidebar (left < sidebar zone) → arrow from RIGHT
    2. Elements near the right edge → arrow from LEFT
    3. For centered elements → pick side with most open space
    4. Prefer horizontal arrows (matching documentation style)
    5. Fall back to vertical only if no horizontal space

    Args:
        layout_config: Optional dict with sidebar_width_css, wp_sidebar_width_css,
                      toolbar_height_css. Uses defaults if not provided.
    ...
    # Read layout constants from config (with sensible defaults)
    sidebar_width = layout_config.get("sidebar_width_css", 100)
    wp_sidebar_width = layout_config.get("wp_sidebar_width_css", 160)
    toolbar_height = layout_config.get("toolbar_height_css", 32)

    # Sidebar zone: the wider of the two sidebars + some margin
    sidebar_zone = max(sidebar_width, wp_sidebar_width) + 40
    ...
    if element_right < sidebar_zone and space_right >= min_space:
        return "right"

    # --- Rule 2: Elements near the top toolbar get arrows from below ---
    if element_bottom < toolbar_height + 20 and space_bottom >= min_space:
        return "bottom"
...
def annotate_screenshot(
    ...
    Args:
        ...
        position: Arrow direction or None for auto.
        layout_config: Optional layout constants dict for arrow positioning.
